import { BadgeCheck, Divide, X } from 'lucide-react'
...
const StoryViewer = ({viewStory, setViewStory}) => {
    ...
    useEffect(()=>{
        ...
        if(viewStory && viewStory.media_type!=='video'){
            ...
            timer = setTimeout(handleCloseStory, duration )
        }
        return ()=> {
            clearTimeout(timer)
    ...
    const renderContent =()=> {
        switch (viewStory.media_type) {
            ...
            case 'video':
                return (
                    <video onEnded={handleCloseStory} src={viewStory.media_url} className='max-h-screen' autoPlay controls/>
                );
            case 'text':
                return (
  ...
  return (
    ...
    <div className='fixed inset-0 z-110 h-screen bg-black bg-opacity-9 flex items-center justify-center'
    style={{backgroundColor: viewStory.media_type=="text"?viewStory.background_color:"#000000"}}>
        ...
        {/* Close Button */}
        <button onClick={handleCloseStory} className='absolute top-4 right-4 text-white text-3xl font-bold focus:outline-none'>
