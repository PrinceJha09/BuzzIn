...
const CreatePost = () => {
  ...
  const handleSubmit=async ()=> {
    if(!images.length && !content){
      return toast.error('Please add atleast one content')
    }
    setLoading(true)
  ...
  return (
    <div className='min-h-screen bg-gradient-to-b from-slate-50 to-white'>
      <div className='max-w-xl mx-1.8 p-6'>{/*kuch apna daala h */}
        {/*Title */}
        <div className='mb-8'>
          <h1 className='text-3xl font-bold text-slate-900 mb-2'>Create Post</h1>
          <p className='text-slate-600'>Buzz your thoughts with the world</p>
        </div>
        ...
        <div className='max-w-xl bg-white p-4 sm:p-8 sm:pb-3 rounded-xl shadow-md space-y-4'>
          ...
          {/*TextArea */}
          <textarea onChange={(e)=>setContent(e.target.value)} value={content} className='w-full resize-none max-h-20 mt-4 text-sm outline-none placeholder-gray-400' placeholder="What's buzzing?"/>
          ...
          <div className='flex items-center justify-between pt-3 border-t border-gray-300'>
            ...
            })} className='text-sm bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 active:scale-95 transition text-white cursor-pointer py-2 px-8 font-medium rounded-md'>Publish Post</button>
          </div>
        </div>
      </div>
